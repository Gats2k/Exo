...
WAVE_BUSINESS_NAME_ID = os.getenv('WAVE_BUSINESS_NAME_ID')  # Requis pour Wave
...
if not WAVE_BUSINESS_NAME_ID:
    logger.warning("WAVE_BUSINESS_NAME_ID non configurée ! Les paiements Wave ne fonctionneront pas.")
...
def initiate_payment():
    ...
    try:
        ...
        # Vérifier la configuration Wave si nécessaire
        if operator == 'wave' and not WAVE_BUSINESS_NAME_ID:
            logger.error("Tentative de paiement Wave sans WAVE_BUSINESS_NAME_ID configuré")
            return jsonify({'error': 'Paiement Wave temporairement indisponible'}), 503
        ...
        # Ajouter business_name_id pour Wave (requis)
        if operator == 'wave':
            easytransfert_data["business_name_id"] = WAVE_BUSINESS_NAME_ID
